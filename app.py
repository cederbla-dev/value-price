...
# --- [공통] 스타일 적용 함수 (분석 3 전용 수정 버전) ---
def apply_v3_style(ax, title, xlabel, ylabel):
    ...
    if run_val and val_ticker:
        ...
        if combined is not None:
            ...
            for base_year in range(2017, 2026):
                ...
                summary_data.append({"Base Year": base_year, "Multiplier": f"{scale_factor:.1f}x", "Fair Value": f"${final_fair:.2f}", "Gap (%)": f"{gap_pct:+.2f}%"})
                fig, ax = plt.subplots(figsize=(7.5, 3.2))
                ax.plot(df_plot.index, df_plot['Close'], label='Price')
                ax.plot(df_plot.index, df_plot['Fair_Value'], linestyle='--', label='Fair Value')
                ax.set_title(f"Base Year: {base_year}"); ax.legend(); plt.xticks(rotation=45)
                st.pyplot(fig)
            st.dataframe(pd.DataFrame(summary_data), width=600)

# --- 2. 개별종목 적정주가 분석 2 ---
elif main_menu == "개별종목 적정주가 분석 2":
    st.title("📅 4분기 단위 적정주가 분석")
    with st.container(border=True):
        ...
        try:
            ...
            for df in dfs:
                if df.iloc[:, 0].astype(str).str.contains('EPS').any():
                    ...
                    raw_eps = target_df[target_df.index.str.contains('EPS')].transpose()
                    raw_eps.index = pd.to_datetime(raw_eps.index, format='%y.%m.%d').sort_index()
                    raw_eps.columns = ['EPS']; break
            price_df = stock.history(start="2017-01-01")['Close'].tz_localize(None)
            ...
            for i in range(0, len(raw_eps) - 3, 4):
                ...
                table_list.append({'기준 연도': f"{group.index[0].year}년", '4분기 EPS합': f"{eps_sum:.2f}", '과거 PER': f"{per:.1f}x", '적정 가치': f"${final_target_eps * per:.2f}"})
            st.dataframe(pd.DataFrame(table_list), width=650, hide_index=True)
        except Exception as e: st.error(f"오류: {e}")

# --- 3. 개별종목 적정주가 분석 3 (요청사항 반영) ---
elif main_menu == "개별종목 적정주가 분석 3":
    st.title("📈 PER 추이 정밀 분석 (평균/중위값)")
    with st.container(border=True):
        col1, col2, col3 = st.columns([1, 1, 2])
        with col1: v3_ticker = st.text_input("🏢 분석 티커", "MSFT", key="v3_t").upper().strip()
        with col2: v3_start_year = st.number_input("📅 시작 연도", 2010, 2025, 2017, key="v3_y")
        with col3: v3_predict_mode = st.radio("🔮 미래 예측", ("None", "현재 분기 예측", "다음 분기 예측"), horizontal=True, index=0, key="v3_p")
    ...
    if run_v3 and v3_ticker:
        try:
            ...
            combined['Label'] = [f"{str(d.year)[2:]}.Q{(d.month-1)//3 + 1}" for d in combined.index]
            ...
            if v3_predict_mode != "None" and stock.earnings_estimate is not None:
                h_eps = combined['EPS'].tolist()
                last_l = plot_df['Label'].iloc[-1]; ly, lq = int("20" + last_l.split('.')[0]), int(last_l.split('Q')[1])
                if v3_predict_mode in ["현재 분기 예측", "다음 분기 예측"]:
                    t1q, t1y = (lq+1, ly) if lq < 4 else (1, ly+1)
                    plot_df.loc[pd.Timestamp(f"{t1y}-{(t1q-1)*3+1}-01")] = [current_price/(sum(h_eps[-3:])+stock.earnings_estimate.loc['0q','avg']), np.nan, f"{str(t1y)[2:]}.Q{t1q}(E)"]
                if v3_predict_mode == "다음 분기 예측":
                    t2q, t2y = (t1q+1, t1y) if t1q < 4 else (1, t1y+1)
                    plot_df.loc[pd.Timestamp(f"{t2y}-{(t2q-1)*3+1}-01")] = [current_price/(sum(h_eps[-2:])+stock.earnings_estimate.loc['0q','avg']+stock.earnings_estimate.loc['+1q','avg']), np.nan, f"{str(t2y)[2:]}.Q{t2q}(E)"]
            avg_per, med_per = plot_df['PER'].mean(), plot_df['PER'].median()
            # 그래프 크기 대폭 축소 (7.5 x 3.5)
            fig, ax = plt.subplots(figsize=(7.5, 3.5), facecolor='white')
            ax.plot(plot_df['Label'], plot_df['PER'], marker='o', color='#0047AB', lw=1.8, ms=5, label='PER Trend (P/TTM EPS)')
            ax.axhline(avg_per, color='#D32F2F', ls='--', lw=1.2, label=f'Mean PER ({avg_per:.2f})')
            ax.axhline(med_per, color='#7B1FA2', ls='-.', lw=1.2, label=f'Median PER ({med_per:.2f})')
            for i, label in enumerate(plot_df['Label']):
                if "(E)" in label: ax.axvspan(i-0.4, i+0.4, color='#FF8C00', alpha=0.1)
            apply_v3_style(ax, f"[{v3_ticker}] PER Analysis: History vs Forecast", "Quarter (Time)", "PER Value")
            ax.legend(loc='upper left', frameon=True, facecolor='white', edgecolor='#d3d3d3', fontsize=7, framealpha=1)
            st.pyplot(fig)
...
# --- 4. 기업 가치 비교 (PER/EPS) ---
    ...
    st.title("⚖️ 기업 가치 비교 분석")
    with st.container(border=True):
        compare_tickers = st.text_input("🏢 비교할 티커들을 입력하세요 (쉼표 구분)", "AAPL, MSFT, GOOGL, AMZN").upper().strip()
        run_compare = st.button("비교 분석 실행", type="primary", use_container_width=True)
    if run_compare and compare_tickers:
        tickers = [t.strip() for t in compare_tickers.split(',')]
        data = []
        for t in tickers:
            s = yf.Ticker(t)
            info = s.info
            data.append({'Ticker': t, 'Price': info.get('currentPrice'), 'PER (Forward)': info.get('forwardPE'), 'EPS (TTM)': info.get('trailingEps'), 'Market Cap': info.get('marketCap')})
        st.table(pd.DataFrame(data))

# --- 5. ETF 섹터 수익률 분석 ---
else:
    st.title("📊 ETF 섹터별 수익률 현황")
    etfs = {'Technology': 'XLK', 'Health Care': 'XLV', 'Financials': 'XLF', 'Energy': 'XLE', 'Consumer Disc': 'XLY'}
    if st.button("ETF 수익률 데이터 업데이트"):
        results = []
        for name, ticker in etfs.items():
            hist = yf.Ticker(ticker).history(period="1y")
            perf = ((hist['Close'].iloc[-1] / hist['Close'].iloc[0]) - 1) * 100
            results.append({'Sector': name, 'Ticker': ticker, '1Y Performance': f"{perf:+.2f}%"})
