...
# --- 메뉴 3: 개별종목 적정주가 분석 3 (사용자 최신 요청 반영: 크기 70%, 범례 보강) ---
elif main_menu == "개별종목 적정주가 분석 3":
    with st.container(border=True):
        col1, col2, col3 = st.columns([1, 1, 2])
        with col1: v3_ticker = st.text_input("🏢 분석 티커 입력", "MSFT").upper().strip()
        with col2: base_year = st.slider("📅 차트 시작 연도", 2017, 2025, 2017)
    ...
    if run_v3 and v3_ticker:
        try:
            ...
            if v3_predict_mode != "None" and est is not None and not est.empty:
                ...
                last_yr = int("20" + last_label.split('.')[0]); last_q = int(last_label.split('Q')[1])
            ...
            avg_per = per_series.mean(); median_per = per_series.median()

            st.subheader(f"📈 {v3_ticker} PER Trend Analysis")
            # 그래프 크기 70% 수준으로 축소 (8.5x4.5)
            ...
            for i, label in enumerate(plot_df['Label']):
                if "(E)" in label: ax.axvspan(i-0.4, i+0.4, color='orange', alpha=0.1)
            ...
            ax.text(0.97, 0.05, info_text, transform=ax.transAxes, verticalalignment='bottom', horizontalalignment='right', fontsize=8, fontweight='bold', bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
            st.pyplot(fig)
...
# --- 메뉴 5: ETF 섹터 수익률 분석 (복구 완료) ---
    ...
    if run_etf_btn and selected_etfs:
        ...
        valid_indices = df_etf.index[df_etf.index >= start_date]
